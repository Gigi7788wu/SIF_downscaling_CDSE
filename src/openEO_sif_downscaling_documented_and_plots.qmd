...
swir = connection.load_collection(
    "SENTINEL3_SYN_L2_SYN",
    spatial_extent=spatial_extent_prototype,
    temporal_extent=temporal_extent_prototype,
    bands=["Syn_S5N_reflectance"],
)

nir_syn = connection.load_collection(
    "SENTINEL3_SYN_L2_SYN",
    spatial_extent=spatial_extent_prototype,
    temporal_extent=temporal_extent_prototype,
...
cube_NDWI = (nir_syn - swir) / (nir_syn + swir)

cube_NDWI = cube_NDWI.rename_labels(dimension="bands", target=["NDWI"])

cube_syn_qc = connection.load_collection(
    "SENTINEL3_SYN_L2_SYN",
    spatial_extent=spatial_extent_prototype,
    temporal_extent=temporal_extent_prototype,
    bands=["CLOUD_flags"],
...
```{python}
mask_syn = cube_syn_qc > 0

cube_NDWI_masked = cube_NDWI.mask(mask_syn)
```
...
cube_NDWI_median = cube_NDWI_masked.reduce_temporal(reducer="median")
...
# cube_NDWI_median.execute_batch(outputfile="data/cube_NDWI_median.tif")
```
...
cube_iwv_local = rio.open_rasterio(filename="data/cube_NDWI_median.tif")
...
cube_iwv_local.plot(ax=axs[3], robust=True)
...
::: {.callout-important}
The LST composite looks heavily influenced by the orbits. For sure it can be better...for example [Terrascope Sentinel-3 Level 3 Land Surface Temperature daily synthesis product](https://docs.terrascope.be/DataProducts/Sentinel-3/ProductsOverview.html)
...
cube_NDWI_median_low = cube_NDWI_median.resample_cube_spatial(
    target=cube_SIF_original_median, method="med"
)
```
...
dataset_SIF_low = dataset_SIF_low.merge_cubes(cube_NDWI_median_low)
...
axs[2].set_title("NDVI")
dataset_sif_low_local["NDWI"].plot(ax=axs[3])
axs[3].set_title("NDWI")
...
param_min = [0, 0.1, -100, -1, -310, 1]

param_ini = [1, 2, 0, 0, -295, 10]

param_max = [1.5, 5, 100, 1, -290, 50]
```
...
regridder = xe.Regridder(parameters_low, lst_high, method="nearest_s2d")
...
cube_NDWI_median_1 = cube_NDWI_median.resample_cube_spatial(target=cube_LST_median)
```
...
cube_to_upscale = cube_to_upscale.merge_cubes(cube_NDWI_median_1)
```
