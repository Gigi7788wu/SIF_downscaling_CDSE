...
```{python}
# | eval: false
# dataset_SIF_low.execute_batch(outputfile="data/sif_predictors_cube_low.tif")
```
...
plt.clf()
nirv_1km_local = rio.open_rasterio("data/cube_nirv_1km_mask.tif", band_as_variable=True)
print(nirv_1km_local)
# creating a mask band for interpolation

nirv_1km_local["mask"] = xr.where(~np.isnan(nirv_1km_local["band_1"]), 1, 0)


nirv_1km_local["mask"].plot()

lst_high = nirv_1km_local.rename_dims(dims_dict={"x": "longitude", "y": "latitude"})
...
# parameters_high = parameters_low.interp_like(lst_high, method="linear")
